...
def print_site_description(spara):
    print ('  + Site:')
    print ('    - ', spara['smc'], ', Soil fertility class:', spara['sfc'])
    print ('  + Stand:') 
    print ('    -', spara['species'], ', vol:', spara['vol'],'m3/ha, age:', spara['age'], 'yrs')
    print ('  + Soil: ')
    if spara['vonP']:
        if 'peat type' in spara and len(spara['peat type']) > 0:
            print ('    - peat top:', spara['peat type'][0], ', von P:', spara['vonP top'][0])
        else:
            print ('    - peat type not specified')
    else:
        if 'peat type' in spara and len(spara['peat type']) > 0:
            print ('    - peat top:', spara['peat type'][0], ', bulk density:', spara['bd top'][0])
        else:
            print ('    - peat type not specified')
    if 'peat type bottom' in spara and len(spara['peat type bottom']) > 0:
        print ('    - peat bottom:', spara['peat type bottom'][0], ', von P:', spara['vonP bottom'])
    else:
        print ('    - bottom peat type not specified')
    print ('  + Drainage:')
...
def print_scenario_nodes(r,c_bals, deltas, ets, h0ts_west, h0ts_east, dwts, bmToYi, g_nuts, end_vols, yi ):
